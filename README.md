...
- 可选接入 Codex、Claude 实时任务：SSH 推送状态变化，合盖使用外接或虚拟显示器时继续，无工作显示器或系统睡眠时暂停
...
- 支持任务完成、等待批准和异常会话提醒
- 可选任务流光，提示本机和远程 Codex、Claude 的任务状态，可调整颜色、速度、亮度和结束时长

### 让长任务安心运行

- 提供手动和随任务两种防睡眠方式，随任务仅跟随本机 Codex
- 可选择等待批准期间继续保持，或同时保持屏幕常亮
- 支持最长防睡眠时间、低电量保护和异常会话保护
- 解除防睡眠后恢复 macOS 电源管理，不主动强制睡眠
