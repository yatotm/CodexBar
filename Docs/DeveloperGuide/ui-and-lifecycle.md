...
`StatusItemController` 消费 `ActivityPresentationModel.statusItemSnapshot`，始终汇总全部工具和设备，主面板标签只筛选面板内容。等待批准优先于运行中；无活跃任务时显示最新终态，10 秒后由单次定时器恢复空闲，不改变最近历史保留期。

`StatusItemIconView` 以 SwiftUI 绘制人物徽章和 Codex 7d 额度圆弧。符号按名称和屏幕倍率缓存为小位图，缩放与交叉淡入淡出复用图片；模板着色适配深浅外观。活跃任务的 tooltip 每分钟更新，同值展示状态不会重新发布。

原生状态按钮统一在 `mouseUp` 切换面板，外部点击监听跳过该窗口的事件，命中范围包含菜单栏上下留白及屏幕顶边，不延伸到相邻状态项。
